...
def create_draft(
    ...
    Args:
        ...
        attachments: Optional list of attachments [{filename, content (base64), mime_type}]
        account_id: Optional ext_connection_id to specify which account to create draft in
    ...
    # Get Gmail service for the specified account, or default if None
    service, connection_id = get_gmail_service(user_id, user_jwt, account_id=account_id)
    ...
    try:
        ...
        # Persist a stable copy so downstream code can recover even if raw_item is later transformed.
        draft_payload = dict(draft)
        draft_payload['gmail_draft_id'] = draft_id
        ...
        # Clean up older drafts in this thread (previous auto-saves)
        if thread_id:
            try:
                cleanup_thread_drafts(
                    ...
                    exclude_external_id=message_id,
                )
            except Exception as cleanup_err:
                logger.warning(f"Draft cleanup failed (non-fatal): {cleanup_err}")
...
def get_draft(
    ...
    Returns:
        Dict with draft details
    """
    # Look up which account owns this draft (critical for multi-account support)
    owner_connection_id = get_email_owner_connection_id(user_id, draft_id, user_jwt)

...
export default function AgentQueuePanel({ workspaceId, workspaceAppId, boardId }: AgentQueuePanelProps) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 z-40 mt-2 w-[min(92vw,430px)] overflow-hidden rounded-2xl border border-[#d8e4f2] bg-white shadow-2xl">
          <div className="flex items-center justify-between border-b border-[#e5edf7] px-3 py-2.5">
            <div className="text-sm font-semibold text-slate-900">Cola de agentes</div>
            <div className="flex items-center gap-1.5">
              <button
                type="button"
                onClick={() => refetch()}
                className="inline-flex h-7 items-center justify-center rounded-lg border border-[#d7e4f2] px-2 text-xs font-medium text-slate-700 hover:bg-slate-50"
                title="Actualizar"
              >
              ...
              <button
                ...
                className="inline-flex h-7 items-center gap-1 rounded-lg border border-[#d7e4f2] px-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50"
              >
          ...
          <div className="max-h-[56vh] overflow-y-auto p-2">
            {jobs.length === 0 ? (
              <div className="rounded-xl border border-dashed border-[#d7e4f2] bg-[#f8fbff] px-3 py-5 text-center text-xs text-slate-500">
                Sin trabajos en cola para este tablero.
              </div>
            ) : (

...
// Fire-and-forget save using keepalive fetch â€” survives page unload
...
export default function FilesView() {
  ...
  const handleDragMove = useCallback(
    (event: DragMoveEvent) => {
      ...
      // Check if over parent drop zone â€” no intent, DroppableFolder's isOver handles visual
      ...
      if (isTargetFolder) {
        ...
        // Still on same folder but timer hasn't fired yet â€” update insert position
      ...
      // Not a folder â€” clear dwell timer and set reorder
      clearDwellTimer();
      dwellTargetRef.current = null;
      setDropIntent({ type: 'reorder', targetId: overId, insertPosition: getInsertPosition() });
    },
  ...
  // Toggle sort â€” click same column flips direction, click new column switches to it desc
  ...
  useEffect(() => {
    ...
    if (doc) {
      ...
      // Documents haven't loaded yet but we have a URL param â€” set the
  ...
  // try to load it directly â€” handles direct links to docs user may or may not have access to.
  ...
  // Save on page leave / tab close â€” uses keepalive fetch to survive unload
  useEffect(() => {
            ...
            <AnimatePresence mode="wait" initial={false}>
              {isLoading ? (
                ...
                >
                  ...
                  >
                  {/* Parent drop zone â€” always rendered when inside a folder to
                  ...
                  >
                  {/* All items â€” folders, notes, files in unified sort order */}
                      ...
                      {documents.map((item, index) => {
                        ...
                        <div key={(item as unknown as { _stableKey?: string })._stableKey || item.id}>
                          ...
                          <SortableDocItem id={item.id}>
                            {({ dragAttributes, dragListeners }) => (
                              itemIsFolder ? (
                                /* â”€â”€ Folder row â€” no DroppableFolder wrapper needed;
                                       folder nesting is handled by handleDragMove zone detection â”€â”€ */
                                    <div
                  ...
                  {/* Drag overlay â€” shows dragged item floating under cursor */}
                  ...
                  <div className="h-full flex items-center justify-between px-6">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <div className="flex items-center gap-2 text-xs text-text-tertiary">
                        ...
                        {isNoteEditable && effectiveSaveStatus === 'saving' && <span>Â· Saving...</span>}
                        {isNoteEditable && effectiveSaveStatus === 'unsaved' && <span className="text-text-secondary">Â· Unsaved</span>}
                        {isNoteEditable && effectiveSaveStatus === 'error' && <span className="text-red-500">Â· Save failed</span>}
                        {isNoteEditable && effectiveSaveStatus === 'saved' && <span>Â· Saved</span>}
                      </div>
                      {!isNoteEditable && (
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="flex-1 flex items-center justify-center relative">
                {accessDenied && urlDocumentId === accessDenied.resourceId ? (
                  ...
                  /* Document is selected but data hasn't loaded yet â€” show skeleton */
